...
namespace Generic
{
	...
	//-------------------------------------------------------------------------
	BOOL CXmlNode::_SetAttribute( CString & strName, CString & strValue IN) const
	{
		ASSERT( m_pNode != NULL );
	...
	CXmlNodePtr CXmlNode::GetChild( CString strName, BOOL bBuildIfNotExist /* = TRUE */)
	{
		...
		{
			TRACE( _T("CXmlNode::GetChildfailed:%s\n"), e.ErrorMessage());
			ASSERT( FALSE );
		}
	...
	CXmlNodePtr CXmlNode::SelectSingleNode(LPCTSTR lpszPath)
	{
		...
		{
			TRACE( _T("CXmlNode::SelectSingleNode failed:%s\n"), e.ErrorMessage());
			ASSERT( FALSE );
		}
	...
	// set long attribute
	BOOL CXmlNode::SetAttribute( CString strName, long lValue)
	{
		CString strValue;
		strValue.Format( _T("%ld"), lValue);
	...
	BOOL CXmlNode::SetAttribute( CString strName, __int64 llValue)
	{
		CString strValue;
		strValue.Format( _T("%I64d"), llValue);
	...
	// set DWORD attribute
	BOOL CXmlNode::SetAttribute( CString strName, DWORD dwValue)
	{
		CString strValue;
		strValue.Format( _T("%lu"), dwValue);
