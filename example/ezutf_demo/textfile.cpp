...
int linebuf_UtoA (WCHAR *u, int len, char **a, int code_page, char *unknown_char, BOOL *data_lost)
{
    if (len < 0)
        len = (int) wcslen (u);
...
// Convert from ANSI to Unicode minimum malloc's
int linebuf_AtoU (char *a, int len, WCHAR **u, int code_page, char *unknown_char, BOOL *data_lost)
{
    if (len < 0)
...
// Constructor
TextFileReader::TextFileReader (TCHAR *filename, HANDLE hFile, int bufsize)
{
...
virtual int TextFileReader::ReadLine
    (TCHAR **line_buf, int code_page, char *unknown_char, BOOL *data_lost)
{
    return TF_EOF;
}
...
template <typename T> int TReadLine (TextFileReader *tf, T **line_buf, bool BE)
{
    ...
    for ( ; ; )
    {
        ...
        if (chars_to_copy > 0 && bp [chars_to_copy - 1] == cr)
            --chars_to_copy;
        tbuf = (T *) alloc_block (tbuf, nchars + chars_to_copy + 1);
...
// Constructor
TextFileWriter::TextFileWriter (TCHAR *filename, HANDLE hFile, int bufsize)
{
    this->filename = filename;
    this->hFile = hFile;
    this->bufsize = (bufsize) ? bufsize : 4096;     // must be even
...
virtual int TextFileWriter::WriteChar
    (TCHAR ch, int code_page, char *unknown_char, BOOL *data_lost)
{
    assert (0);
    return TF_ERROR;
}
...
virtual int TextFileWriter::WriteString
    (TCHAR *s, int len, int code_page, char *unknown_char, BOOL *data_lost)
{
    assert (0);
    return TF_ERROR;
}
...
// WriteBytes
int TextFileWriter::WriteBytes (BYTE *bytes, int nbytes)
{
    for ( ; ; )
    {
...
// Constructor
ANSI_TextFileReader::ANSI_TextFileReader (TextFile *tf, TCHAR *filename,
...
// Constructor
UTF16LE_TextFileReader::UTF16LE_TextFileReader (TCHAR *filename, HANDLE hFile, int bufsize) :
...
// Constructor
UTF16BE_TextFileReader::UTF16BE_TextFileReader (TCHAR *filename, HANDLE hFile, int bufsize) :
    TextFileReader (filename, hFile, bufsize)
{
    assert ((bufsize & 1) == 0);            // avoid buffer boundary problems
#ifndef _UNICODE
...
// Constructor
UTF8_TextFileReader::UTF8_TextFileReader (TCHAR *filename, HANDLE hFile, int bufsize) :
    TextFileReader (filename, hFile, bufsize)
{
    this->utf8_buf = NULL;
#ifndef _UNICODE
...
// Constructor
ANSI_TextFileWriter::ANSI_TextFileWriter (TCHAR *filename, HANDLE hFile, int bufsize) :
...
// WriteChar
int ANSI_TextFileWriter::WriteChar (TCHAR ch, int code_page, char *unknown_char, BOOL *data_lost)
{
#ifdef _UNICODE
    WCHAR u [2];
    u [0] = ch;
...
// Constructor
UTF16LE_TextFileWriter::UTF16LE_TextFileWriter (TCHAR *filename, HANDLE hFile, int bufsize) :
    TextFileWriter (filename, hFile, bufsize)
{
#ifndef _UNICODE
    this->utf16_buf = NULL;
...
// Constructor
UTF16BE_TextFileWriter::UTF16BE_TextFileWriter (TCHAR *filename, HANDLE hFile, int bufsize) :
    TextFileWriter (filename, hFile, bufsize)
{
    this->utf16_buf = NULL;
}
...
// Convert one Unicode character to upto 3 UTF8 bytes
// Returns byte count
static inline int UtoUTF8 (WCHAR ch, BYTE b [3])
{
    ...
    if (ch < 0x800)             // U-0080 - U-07FF: 110bbbbb 10bbbbbb
    {
        ...
        return 2;
                                ...
                                // U-0800 - U-FFFF: 1110bbbb 10bbbbbb 10bbbbbb
    b [0] = (ch >> 12) | 0xe0;
    b [1] = ((ch >> 6) & 0x3F) | 0x80;
    b [2] = (ch & 0x3F) | 0x80;
    return 3;
...
// Constructor
UTF8_TextFileWriter::UTF8_TextFileWriter (TCHAR *filename, HANDLE hFile, int bufsize) :
    TextFileWriter (filename, hFile, bufsize) {}
...
// WriteChar
int UTF8_TextFileWriter::WriteChar (TCHAR ch, int code_page, char *unknown_char, BOOL *data_lost)
{
#ifdef _UNICODE
    WCHAR w = ch;
#else
    WCHAR w = (BYTE) ch;                    // avoid sign-extension problems
#endif

    BYTE b [3];
    int nbytes = UtoUTF8 (w, b);
    ...
    (TCHAR *s, int len, int code_page, char *unknown_char, BOOL *data_lost)
{
    while (len--)                               // inline, for speed
    {
        ...
        WCHAR w = *s++;
#else
        WCHAR w = (BYTE) *s++;                  // avoid sign-extension problems
#endif
        BYTE b [3];
        int nbytes = UtoUTF8 (w, b);
...
int TextFile::Open (TCHAR *filename, int filemode, int encoding, int bufsize)
{
...
{
    if (this->encoding == TF_ANSI)          // force ANSI mode
    {
