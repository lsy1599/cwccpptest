...
void SetLastErrorString (TCHAR *err)
{
...
void vLogError (TCHAR *format, va_list ap)
{
...
// Log an error message
void _cdecl LogError (TCHAR *format, ...)
{
    va_list ap;
    va_start (ap, format);
...
TCHAR * vFormatPString (TCHAR *message, va_list ap)
{
    TCHAR buf1 [T2B (4096)];
    TCHAR buf2 [T2B (4096)];
...
TCHAR * _cdecl FormatPString (TCHAR *message, ...)
{
    va_list ap;
    va_start (ap, message);
