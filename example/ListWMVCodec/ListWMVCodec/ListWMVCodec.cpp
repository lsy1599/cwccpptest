// ListWMVCodec.cpp : 定義主控台應用程式的進入點。
//
...
CStdioFile f(L"d:\\test_log.txt", CFile::modeCreate | CFile::modeWrite | CFile::typeText ) ;

f.WriteString(L"test");
f.Close();

HRESULT    hr;
    CComPtr<IWMProfileManager>    pIWMProfileMgr;
    hr = WMCreateProfileManager( &pIWMProfileMgr );   

    CComQIPtr<IWMCodecInfo>    pCodecInfo = pIWMProfileMgr;
    CComQIPtr<IWMCodecInfo2> pCodecInfo2 = pIWMProfileMgr;

    DWORD dwCodecCount=0;
    hr = pCodecInfo->GetCodecInfoCount( WMMEDIATYPE_Video, &dwCodecCount );
CODEC_VIDEO_MPEG43
