	...
	BOOL      enable)
{
    ...
    NTSTATUS   Status;
	BOOL       failed = TRUE;
	char       asciiAccountName[256];
	WCHAR      AccountName[256];

	popstring(asciiAccountName);
...
/*
  ...
  MODS:                        given user.
*/
void __declspec(dllexport) HasLogonAsAService(
	HWND      hwndParent, 
	int       string_size, 
	char*     variables, 
	stack_t** stacktop)
        ...
        &pSid))     // buffer to allocate to contain resultant SID
	{
			...
			&count)) == STATUS_SUCCESS)
		{
			...
			for (i = 0; found == FALSE && i < count; i++)
			{
				...
				if (0 == stricmp(ascii,"SeServiceLogonRight"))
					found = TRUE;
			}
...
/*
  ...
  USGE:   ; $0 now contains either 'Yes', 'No' or an error description
*/
void __declspec(dllexport) GrantLogonAsAService(
	HWND      hwndParent, 
	int       string_size, 
	char*     variables, 
	stack_t** stacktop)
...
/*
  ...
  USGE:   ; $0 now contains either 'Yes', 'No' or an error description
*/
void __declspec(dllexport) RemoveLogonAsAService(
	HWND      hwndParent, 
	int       string_size, 
	char*     variables, 
	stack_t** stacktop)
